...
pub struct TrackedMints {
    ...
    pub bump: u8,
    pub reserved: [u8; 127],
    pub st_mint_list: [VaultEntry; 64],
}
...
impl TrackedMints {
    ...
    pub fn new(ncn: Pubkey, bump: u8) -> Self {
        Self {
            ...
            bump,
            reserved: [0; 127],
            st_mint_list: [VaultEntry::default(); MAX_VAULT_OPERATOR_DELEGATIONS],
        }
    }
    ...
    pub fn add_mint(&mut self, st_mint: Pubkey, vault_index: u64) -> Result<(), ProgramError> {
        ...
        *mint_entry = VaultEntry::new_grouped(st_mint, vault_index);
        Ok(())
    }
    ...
    pub fn get_mint_entry(&self, vault_index: u64) -> Result<VaultEntry, ProgramError> {
        let mint_entry = self
            .st_mint_list
            .iter()
            .find(|m| m.vault_index() == vault_index)
